...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *sselectedDate = [dateFormat stringFromDate:selectedDate];
    // NSDate *day = [self.gregorian dateByAddingUnit:NSCalendarUnitDay value:1 toDate:selectedDate options:0];
    NSUserDefaults *pref = [NSUserDefaults standardUserDefaults];
    [pref setObject:sselectedDate forKey:@"SelectedCalendarDate"];
    [pref synchronize];
    NSLog(@"1Date - %@",sselectedDate);
    
    BOOL isToday = [self.gregorian isDateInToday:selectedDate];
    NSLog(@"Test istoday %d",isToday);
    BOOL sameDay = [self.gregorian isDate:[NSDate date] inSameDayAsDate:selectedDate];
    NSLog(@"Test Sameday %d",sameDay);
